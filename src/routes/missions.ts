...
const mission07StartSchema = z
  .object({
    seed: z.number().int().nonnegative().default(MISSION_07_DEFAULT_SEED)
  })
  .strict();

const mission07ResolveSchema = z
  .object({
    schemaVersion: z.literal(1).default(1),
    seed: z.number().int().nonnegative(),
...
const mission10StartSchema = z
  .object({
    seed: z.number().int().nonnegative().default(MISSION_10_DEFAULT_SEED)
  })
  .strict();
...
export function registerMissionRoutes(app: FastifyInstance) {
  ...
  app.post('/missions/mission-07-burning-seas/start', async (request, reply) => {
    if (!(await ensureFlag(app, reply, 'missions_api', { missionCode: MISSION_07_CODE }))) {
      return;
    }

    const parsed = mission07StartSchema.safeParse(request.body ?? {});
    if (!parsed.success) {
      return reply.status(400).send({ error: parsed.error.format() });
    }

    return mission07StartResponse(parsed.data.seed);
  });

  app.post('/missions/mission-07-burning-seas/resolve', async (request, reply) => {
    if (!(await ensureFlag(app, reply, 'missions_api', { missionCode: MISSION_07_CODE }))) {
      return;
    }

    const parsed = mission07ResolveSchema.safeParse(request.body);
    if (!parsed.success) {
      return reply.status(400).send({ error: parsed.error.format() });
    }
    ...
    const playerShipIds = new Set<string>(MISSION_07_PLAYER_SHIP_IDS);
    const enemyShipIds = new Set<string>(MISSION_07_ENEMY_SHIP_IDS);
    for (const turnOrders of parsed.data.turns) {
      for (const order of turnOrders) {
        if (!playerShipIds.has(order.shipId)) {
          return reply.status(400).send({ error: 'invalid_order_ship', shipId: order.shipId });
    ...
    request.log.info(
      {
        ...
        damageProfile: outcome.damageProfile,
        bonusObjectives: outcome.bonusObjectives,
        ignitionsInflicted: outcome.telemetry.ignitionsInflicted,
        ignitionsSuffered: outcome.telemetry.ignitionsSuffered,
        slowsInflicted: outcome.telemetry.slowsInflicted,
        requestId: request.id
      },
      'mission07_resolved'
    );
  ...
  app.post('/missions/mission-10-sail-cutter/start', async (request, reply) => {
    if (!(await ensureFlag(app, reply, 'missions_api', { missionCode: MISSION_10_CODE }))) {
      return;
    }

    const parsed = mission10StartSchema.safeParse(request.body ?? {});
    if (!parsed.success) {
      return reply.status(400).send({ error: parsed.error.format() });
    }

    return mission10StartResponse(parsed.data.seed);
  });

  app.post('/missions/mission-10-sail-cutter/resolve', async (request, reply) => {
    if (!(await ensureFlag(app, reply, 'missions_api', { missionCode: MISSION_10_CODE }))) {
      return;
    }

    const parsed = mission10ResolveSchema.safeParse(request.body);
    if (!parsed.success) {
      return reply.status(400).send({ error: parsed.error.format() });
    }
    ...
    const playerShipIds = new Set<string>(MISSION_10_PLAYER_SHIP_IDS);
    const enemyShipIds = new Set<string>(MISSION_10_ENEMY_SHIP_IDS);
    for (const turnOrders of parsed.data.turns) {
      for (const order of turnOrders) {
        if (!playerShipIds.has(order.shipId)) {
          return reply.status(400).send({ error: 'invalid_order_ship', shipId: order.shipId });
    ...
    request.log.info(
      {
        ...
        damageProfile: outcome.damageProfile,
        bonusObjectives: outcome.bonusObjectives,
        chainShotOrders: outcome.telemetry.chainShotOrders,
        chainShotHits: outcome.telemetry.chainShotHits,
        roundShotHits: outcome.telemetry.roundShotHits,
        chainSailDamageDealt: outcome.telemetry.chainSailDamageDealt,
        requestId: request.id
      },
      'mission10_resolved'
    );
