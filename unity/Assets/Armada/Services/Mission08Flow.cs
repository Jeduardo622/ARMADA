...
{
    public sealed class Mission08FlowResult
    {
        public bool Success { get; init; }
        public string FailureReason { get; init; }
        public Mission08Outcome Outcome { get; init; }
    }

    /// <summary>
    /// Drives the Mission 08 "Eye of the Wind" client flow: applies the
    ...
    /// payload shape.
    /// </summary>
    public sealed class Mission08Flow
    {
        private readonly IMission08Client _client;
        ...
        public Mission08Flow(
            IMission08Client client,
        ...
        public async Task<Mission08FlowResult> RunAsync(int seed, List<List<SimOrder>> turns)
        {
            ...
            var start = await _client.StartMission08Async(seed);
            ...
            if (Mission08Scenario.FingerprintOf(start.Data) != Mission08Scenario.Fingerprint())
            {
            ...
            var resolve = await _client.ResolveMission08Async(new Mission01ResolveRequest
            {
                Seed = seed,
                Turns = runTurns
            });
            ...
            if (resolve.Data.Seed != seed || resolve.Data.MissionCode != Mission08Scenario.MissionCode)
            {
            ...
            return new Mission08FlowResult { Success = true, Outcome = resolve.Data };
        }
        ...
        public async Task<ServiceResult<MissionCompleteResponse>> CompleteAsync(string playerId, Dictionary<string, object> result, int? bestScore = null)
        {
            ...
            return await _completionClient.CompleteAsync(Mission08Scenario.MissionCode, new MissionCompleteRequest
            {
                PlayerId = playerId,
                Result = result,
                BestScore = bestScore,
        ...
        private static Mission08FlowResult Fail(string reason)
        {
            return new Mission08FlowResult { Success = false, FailureReason = reason };
        }
    }
}
