...
namespace Armada.Client.Services
{
    ...
    public interface IMission08Client
    {
        Task<ServiceResult<Mission08StartResponse>> StartMission08Async(int seed);
        Task<ServiceResult<Mission08Outcome>> ResolveMission08Async(Mission01ResolveRequest request);
    }

    public interface IMission09Client
    {
        Task<ServiceResult<Mission09StartResponse>> StartMission09Async(int seed);
    ...
    public sealed class MissionService : IMission01Client, IMission02Client, IMission03Client, IMission04Client, IMission05Client, IMission06Client, IMission07Client, IMission08Client, IMission09Client, IMission10Client, IMissionCompletionClient
    {
        ...
        public async Task<ServiceResult<Mission08StartResponse>> StartMission08Async(int seed)
        {
            var resp = await _client.SendAsync<Mission08StartResponse>($"/missions/{Mission08Scenario.MissionCode}/start", UnityWebRequest.kHttpVerbPOST, new Mission01StartRequest { Seed = seed });
            var featureDisabled = false;
            if (resp.StatusCode == HttpStatusCode.Forbidden)
            {
                _flags.DisableFromForbidden(FeatureKey);
            ...
            return ServiceResult<Mission08StartResponse>.FromResponse(resp, featureDisabled);
        }

        public async Task<ServiceResult<Mission08Outcome>> ResolveMission08Async(Mission01ResolveRequest request)
        {
            var resp = await _client.SendAsync<Mission08ResolveEnvelope>($"/missions/{Mission08Scenario.MissionCode}/resolve", UnityWebRequest.kHttpVerbPOST, request);
            var featureDisabled = false;
            if (resp.StatusCode == HttpStatusCode.Forbidden)
            {
                _flags.DisableFromForbidden(FeatureKey);
            ...
            return new ServiceResult<Mission08Outcome>
            {
                Data = resp.Data?.Outcome,
                Success = resp.Success,
                Status = resp.StatusCode,
        ...
        public async Task<ServiceResult<Mission09StartResponse>> StartMission09Async(int seed)
        {
            var resp = await _client.SendAsync<Mission09StartResponse>($"/missions/{Mission09Scenario.MissionCode}/start", UnityWebRequest.kHttpVerbPOST, new Mission01StartRequest { Seed = seed });
            var featureDisabled = false;
            if (resp.StatusCode == HttpStatusCode.Forbidden)
            {
                _flags.DisableFromForbidden(FeatureKey);
            ...
            return ServiceResult<Mission09StartResponse>.FromResponse(resp, featureDisabled);
        }

        public async Task<ServiceResult<Mission09Outcome>> ResolveMission09Async(Mission01ResolveRequest request)
        {
            var resp = await _client.SendAsync<Mission09ResolveEnvelope>($"/missions/{Mission09Scenario.MissionCode}/resolve", UnityWebRequest.kHttpVerbPOST, request);
            var featureDisabled = false;
            if (resp.StatusCode == HttpStatusCode.Forbidden)
            {
                _flags.DisableFromForbidden(FeatureKey);
