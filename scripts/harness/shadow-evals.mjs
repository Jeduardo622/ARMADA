...
export async function writeShadowReports(root, report, hooks = {}) {
  ...
  const directory = join(resolve(root), "reports", "harness", "shadow");
  ...
  const summary = [`# Shadow Agent Evaluation`, "", `Status: **${escapeMarkdown(report.status)}**`, `Aggregate score: **${report.aggregateScore ?? "n/a"}**`, `Critical misses: **${report.criticalMisses.length}**`, "", "| Fixture | Score | Policy mismatches | Critical misses |", "| --- | ---: | ---: | ---: |", ...rows, ""].join("\n").slice(0, 32_768);
...
async function main() {
  ...
  const model = modelIndex >= 0 && args[modelIndex + 1]
    ? args[modelIndex + 1]
    : (args[modeIndex] === "--replay" ? "replay" : "gpt-5.3-codex");
  const root = resolve(fileURLToPath(new URL("../..", import.meta.url)));
  const publicSuite = JSON.parse(await readFile(join(root, "tests/harness/fixtures/shadow-evals.json"), "utf8"));
  const privateExpectations = JSON.parse(await readFile(join(root, "tests/harness/fixtures/shadow-expectations.json"), "utf8"));
  const benchmarkLock = JSON.parse(await readFile(join(root, "tests/harness/fixtures/shadow-benchmark-lock.json"), "utf8"));
  ...
  const report = gradeShadowEvaluation({ suite, response: raw, metadata: { model, commitSha: process.env.GITHUB_SHA ?? "local", workflowRunId: process.env.GITHUB_RUN_ID ?? "local", timestamp: new Date().toISOString(), upstreamStatus } });
