...
export async function encodeShadowResponse(inputPath, githubOutputPath) {
  ...
  try {
    ...
    if (response.length < 1 || response.length > MAX_SHADOW_RESPONSE_BYTES) {
      throw new Error(`Shadow response size ${response.length} is outside the 1..${MAX_SHADOW_RESPONSE_BYTES} byte transport bound`);
    }
...
export function decodeShadowResponse(encoded) {
  if (typeof encoded !== "string" || encoded.length === 0 || encoded.length % 4 !== 0 || !BASE64_PATTERN.test(encoded)) {
    throw new Error("Shadow response transport envelope is missing or malformed");
  }
  ...
  if (response.length < 1 || response.length > MAX_SHADOW_RESPONSE_BYTES || response.toString("base64") !== encoded) {
    throw new Error(`Decoded Shadow response is outside the 1..${MAX_SHADOW_RESPONSE_BYTES} byte transport bound`);
  }
  return response;
}
...
async function main() {
  ...
  if (command === "decode") {
    await writeDecodedShadowResponse(process.env.SHADOW_RESPONSE_B64 || "", argument("--output"));
    return;
  }
  if (command === "combine") {
  ...
  throw new Error("usage: shadow-transport.mjs <encode|decode|combine> [options]");
}
