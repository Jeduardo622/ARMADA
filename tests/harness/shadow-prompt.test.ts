...
function expectedPrompt(): string {
  const base = readFileSync(".github/prompts/shadow-evals.md", "utf8").trimEnd();
  ...
  it("builds the exact ordered public prompt without private grader data", async () => {
    ...
    try {
      execFileSync(process.execPath, [
        resolve("scripts/harness/build-shadow-prompt.mjs"),
        "--source-root",
        resolve("."),
        "--output",
        output,
      ...
      expect(prompt).not.toContain("shadow-expectations.json");
      expect(prompt).not.toContain("shadow-responses.json");
      expect(prompt).not.toContain(readFileSync("tests/harness/fixtures/shadow-expectations.json", "utf8"));
      expect(prompt).not.toContain(readFileSync("tests/harness/fixtures/shadow-responses.json", "utf8"));
